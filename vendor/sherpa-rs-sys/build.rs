...
fn main() {
    ...
    // Prepare sherpa-onnx source. Skip when the C++ submodule is absent
    // (minimum-vendor mode: only the Rust binding files are committed; the
    // upstream sherpa-onnx tree lives outside the repo, and pre-built .so
    // libs are linked via SHERPA_LIB_PATH).
    if sherpa_src.exists() && !sherpa_dst.exists() {
    ...
    // Bindings — use pre-generated src/bindings.rs when sherpa-onnx C++
    // headers are absent (minimum-vendor mode) or when explicitly requested.
    if env::var("SHERPA_SKIP_GENERATE_BINDINGS").is_ok() || !sherpa_src.exists() {
        debug_log!("Skip generate bindings (using pre-generated src/bindings.rs)");
