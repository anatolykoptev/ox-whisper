    ...
    pub fn new(config: KeywordSpotConfig) -> Result<Self> {
        ...
        let sherpa_config = unsafe {
            sherpa_rs_sys::SherpaOnnxKeywordSpotterConfig {
                ...
                model_config: {
                    let mut mc: sherpa_rs_sys::SherpaOnnxOnlineModelConfig = unsafe { mem::zeroed() };
                    mc.transducer.encoder = zipformer_encoder.as_ptr();
                    mc.transducer.decoder = zipformer_decoder.as_ptr();
                    mc.transducer.joiner = zipformer_joiner.as_ptr();
                    mc.num_threads = config.num_threads.unwrap_or(1);
                    mc.provider = provider.as_ptr();
                    mc.debug = config.debug.into();
                    mc.tokens = tokens.as_ptr();
                    mc
                },
            }
        };
