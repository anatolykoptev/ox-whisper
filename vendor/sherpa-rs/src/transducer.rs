...
impl TransducerRecognizer {
    pub fn new(config: TransducerConfig) -> Result<Self> {
        let recognizer = unsafe {
            ...
            let offline_model_config = unsafe {
                let mut c: sherpa_rs_sys::SherpaOnnxOfflineModelConfig = mem::zeroed();
                c.transducer.encoder = encoder.as_ptr();
                c.transducer.decoder = decoder.as_ptr();
                c.transducer.joiner = joiner.as_ptr();
                c.tokens = tokens.as_ptr();
                c.num_threads = config.num_threads;
                c.debug = debug;
                c.provider = provider_ptr.as_ptr();
                c.model_type = model_type.as_ptr();
                c.modeling_unit = modeling_unit.as_ptr();
                c.bpe_vocab = bpe_vocab.as_ptr();
                c
            };
