...
- [ ] `POST /v1/audio/transcriptions` (multipart: file, model, language, response_format, temperature, timestamp_granularities)
- [ ] Форматы ответа: `json`, `verbose_json`, `text`, `srt`, `vtt`
- [ ] `verbose_json`: segments с start/end/text + word-level timestamps
- [ ] SRT/VTT генерация из word timestamps
- [ ] Авто-определение языка (language detection) когда `language` не указан
...
- [ ] `/v1/models` — список загруженных моделей и языков
...
| Feature | Deepgram | AssemblyAI | Gladia | ox-whisper v0.1 | Цель |
|---------|----------|------------|--------|-----------------|------|
| Self-hosted | $$$$ | нет | нет | **да** | — |
| OpenAI API compat | нет | нет | нет | нет | v0.2 |
| SRT/VTT | да | да | да | нет | v0.2 |
| Language detection | да | да | да | нет | v0.2 |
