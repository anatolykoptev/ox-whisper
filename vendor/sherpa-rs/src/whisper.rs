...
impl WhisperRecognizer {
    pub fn new(config: WhisperConfig) -> Result<Self> {
        ...
        let model_config = unsafe {
            ...
            c.whisper.decoder = decoder_ptr.as_ptr();
            c.whisper.encoder = encoder_ptr.as_ptr();
            c.whisper.language = language_ptr.as_ptr();
            c.whisper.task = task_ptr.as_ptr();
            c.whisper.tail_paddings = tail_paddings;
            c.debug = debug;
            c.num_threads = num_threads;
            c.provider = provider_ptr.as_ptr();
            c.bpe_vocab = bpe_vocab_ptr.as_ptr();
